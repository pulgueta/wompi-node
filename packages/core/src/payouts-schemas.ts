...
/** A plain string on the REST API, but `{ code, message }` on webhook events. */
export const PayoutFailureReasonSchema = z.union([
  z.string(),
  z.object({ code: z.string().optional(), message: z.string().optional() }).loose(),
]);
...
/** Beneficiary shape emitted by Payouts `transaction.updated` webhooks. */
