...
The request below sets `personType` explicitly. Wompi's OpenAPI contract
includes it while Wompi's prose example omits it, so the SDK keeps it optional
and doesn't infer a value.
...
sandbox-only, and the SDK rejects them client-side outside the sandbox — switch
`sandbox: false`, and swap in the production API key, principal ID and events
secret. Everything else stays identical.
:::
