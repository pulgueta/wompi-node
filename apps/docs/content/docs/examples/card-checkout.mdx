...
**Live** The form below calls the same SDK methods you'd use in production. The docs server supplies its sandbox credentials, so you can charge a test card without entering any keys. Check the framework examples to adapt the route to your stack.

:::note[Sandbox only]
The route hard-codes `sandbox: true`. `4242 4242 4242 4242` is approved;
`4111 1111 1111 1111` is declined. The form never accepts or exposes Wompi credentials.
:::
